...
* Afficher le tableau de bord vulgarisé reprenant la métaphore du **"Contrôle Technique Automobile" 🚗** (Vignette, Freins coupés, Pneus lisses, Anomalie carrosserie).

## ⚙️ Fonctionnalités Avancées Intégrées
* **Génération PDF :** Utilisation de `pdfkit` (ou équivalent) pour transformer le rapport HTML (le bilan du contrôle technique) en un document téléchargeable.
* **Serveur Mail (`smtplib`) :** Envoi du rapport final directement sur la boîte mail du client.
* **Job Scheduler (`APScheduler`) :** Automatisation d'une tâche (Cronjob) permettant de relancer l'audit de base tous les jours à 8h00 et d'avertir le client de l'état de son véhicule numérique.
